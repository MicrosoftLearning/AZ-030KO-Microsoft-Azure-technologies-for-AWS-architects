...
서버리스는 초당 사용되는 컴퓨팅에 대한 워크로드 수요 및 청구에 따라 컴퓨팅을 자동으로 크기 조정하는 개별 Azure SQL Databases 인스턴스의 컴퓨팅 계층입니다. 또한, 서버리스 컴퓨팅 계층은 활동이 반환될 때 스토리지만 청구되고 데이터베이스를 자동으로 다시 시작하는 경우 비활성 기간 동안 데이터베이스를 자동으로 일시 중지할 수 있습니다.
...
1. **SQL Database 만들기** 블레이드의 **기본** 탭으로 돌아가서 **다음: 네트워킹 >** 을 선택합니다. 
...
1. **SQL Database 만들기** 블레이드의 **추가 설정** 탭에서 다음 설정을 지정합니다(다른 설정은 기본 값으로 남겨둡니다).
    ...
    | --- | --- |
    | 기존 데이터 사용 | **샘플** |
    | Advanced Data Security 사용하도록 설정 | **나중에** |
