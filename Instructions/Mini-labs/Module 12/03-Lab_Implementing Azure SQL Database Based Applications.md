...
Adatum Corporation은 .NET 코어 기반 프런트 엔드 및 SQL Server 기반 백 엔드가 있는 2계층 애플리케이션을 보유하고 있습니다. Adatum Enterprise의 아키텍처 팀은 Azure SQL Database를 데이터 계층으로 활용하여 이러한 애플리케이션을 구현할 수 있는 가능성을 살펴보고 있습니다. 기존 SQL Server 백 엔드의 일시적이며 예측할 수 없는 사용과 프런트 엔드 앱에 기본 제공된 대기 시간의 상대적으로 높은 허용 범위 때문에 Adatum은 Azure SQL Database의 서버리스 계층을 고려하고 있습니다. 

서버리스는 초당 사용되는 컴퓨팅에 대한 워크로드 수요 및 청구서에 따라 컴퓨팅을 자동으로 크기 조정하는 개별 Azure SQL Databases 인스턴스의 컴퓨팅 계층입니다. 또한, 서버리스 컴퓨팅 계층은 활동이 반환될 때 스토리지만 청구되고 데이터베이스를 자동으로 다시 시작하는 경우 비활성 기간 동안 데이터베이스를 자동으로 일시 중지할 수 있습니다.

또한, Adatum 엔터프라이즈 아키텍처 팀은 앱이 사이트 간 VPN 또는 ExpressRoute를 통한 하이브리드 연결에 의존하지 않고 온-프레미스 위치에서 연결할 수 있어야 하는 시나리오에서 특정 IP 주소 범위로 인바운드 연결을 제한할 수 있도록 Azure SQL Databases에서 제공하는 네트워크 수준 보안을 평가하는 데 관심이 있습니다. 
  
Adatum 아키텍처 팀은 이러한 목표를 달성하기 위해 다음과 같은 Azure SQL 데이터베이스 기반 애플리케이션을 테스트합니다:

-  Azure SQL 데이터베이스의 서버리스 계층 구현
...
1. 랩 컴퓨터에서 웹 브라우저를 시작하고 [Azure Portal](https://portal.azure.com)로 이동하고 이 랩에서 사용할 구독에서 소유자 역할을 가진 사용자 계정의 자격 증명을 제공하여 로그인합니다.

1. Azure Portal에서 **SQL Database**를 검색 및 선택하고 **SQL Database** 블레이드에서 **+ 추가**를 선택합니다.

1. **SQL Database 만들기** 블레이드의 **기본** 탭에서 다음 설정을 지정합니다(다른 설정은 기본값으로 남겨둡니다).
...
1. **서버** 드롭다운 목록 바로 아래에서 **새로 만들기**를 선택하고 **새 서버** 블레이드에서 다음 설정을 지정하고 **확인**을 선택합니다(다른 설정은 기본값으로 남겨둡니다).

    | 설정 | 값 | 
    | --- | --- |
    | 서버 이름 | 전역적으로 유효하고 고유한 이름 | 
    | 서버 관리자 로그인 | **sqladmin** |
    | 암호 | **Pa55w.rd1234** |
...
1. **구성** 블레이드에서 **서버리스**를 선택하고 해당 하드웨어 구성 및 자동 일시 중지 지연 설정을 검토하고 **자동 일시 중지 사용** 확인란을 사용하도록 설정하고 **적용**을 선택합니다.

1. **SQL Database 만들기** 블레이드의 **기본** 탭으로 돌아가서 **다음:**을 선택합니다. **네트워킹 >**. 

1. **SQL Database 만들기** 블레이드의 **네트워킹** 탭에서 다음 설정을 지정합니다(다른 설정은 기본값으로 남겨둡니다).
...
1. 페이지 맨 아래에서 **다음: 추가 설정 >**. 

1. **SQL Database 만들기** 블레이드의 **추가 설정** 탭에서 다음 설정을 지정합니다(다른 설정은 기본 값으로 남겨둡니다).

    | 설정 | 값 | 
    | --- | --- |
    | 기존 데이터 사용 | **샘플** |
    | Advanced Data Security 사용하도록 설정 | **시기상조** |
...
### 연습 2: Azure SQL Database를 데이터 저장소로 사용하는 .NET 코어 콘솔 앱 구현
  
이 연습의 주요 작업은 다음과 같습니다.

1. Azure SQL Database의 ADO.NET 연결 정보 식별

1. .NET 코어 콘솔 앱 만들기 및 구성

1. .NET 코어 콘솔 앱 테스트

1. Azure SQL Database 방화벽 구성

1. .NET 코어 콘솔 앱의 기능 확인
...
1. **Bash** 또는 **PowerShell**을 선택하라는 메시지가 표시되면 **Bash**를 선택합니다. 
    ...
    >**참고**: **Cloud Shell**을 처음 시작하고 **탑재된 스토리지 없음**이라는 메시지가 표시되면 이 랩에서 사용하는 구독을 선택하고 **스토리지 만들기**를 선택합니다. 
...
1. **방화벽 설정** 블레이드에서 다음 항목을 설정하고 **저장**을 선택합니다.
    ...
    >**참고**: Cloud Shell 세션을 다시 시작한 후에 IP 주소가 변경되므로 이는 분명 랩 용도 전용입니다.


#### 작업 5: .NET 코어 콘솔 앱의 기능 확인
...
1. Cloud Shell 창에서 다음을 실행하여 이 연습에서 만든 리소스 그룹을 나열합니다.
   ...
   ```
    ...
    > **참고**: 이 랩에서 만든 리소스 그룹만 출력에 포함되어 있는지 확인합니다. 해당 작업에서는 이러한 그룹을 삭제할 것입니다.
